...
import com.mycompany.myapp.service.PilotoQueryService;
...
public class PilotoResource {
    ...
    private final PilotoQueryService pilotoQueryService;

    public PilotoResource(PilotoService pilotoService, PilotoRepository pilotoRepository, PilotoQueryService pilotoQueryService) {
        this.pilotoService = pilotoService;
        this.pilotoRepository = pilotoRepository;
        this.pilotoQueryService = pilotoQueryService;
    }
    ...
    public ResponseEntity<List<Piloto>> getAllPilotos(
        PilotoCriteria criteria,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to get Pilotos by criteria: {}", criteria);
        Page<Piloto> page = pilotoQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /pilotos/count} : count all the pilotos.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/pilotos/count")
    public ResponseEntity<Long> countPilotos(PilotoCriteria criteria) {
        log.debug("REST request to count Pilotos by criteria: {}", criteria);
        return ResponseEntity.ok().body(pilotoQueryService.countByCriteria(criteria));
    }
